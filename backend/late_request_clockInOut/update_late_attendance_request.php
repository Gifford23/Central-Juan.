...
/* ---------- helpers (from admin logic, aligned with create_attendance.php) ---------- */
...
/* ---------- DB helpers (unchanged, preserved) ---------- */
...
/* ---------- block-aware attendance processing (aligned to create_attendance.php) ---------- */
/**
 * Returns array with success and debug.
 * Accepts optional $override_work_time_id to force using the request's work_time row.
 */
function process_block_aware_attendance($conn, $employee_id, $employee_name, $attendance_date, $time_in_morning, $time_out_morning, $time_in_afternoon, $time_out_afternoon, $override_work_time_id = null) {
    ...
    // mobile normalization (keep existing subtle corrections)
    ...
    // build break intervals clipped to shift and compute mapped break minutes (use floor)
    $breakIntervals = [];
    ...
    foreach ($breaks as $br) {
        ...
        if ($ov > 0) {
            ...
            // use floor (consistent with create_attendance)
    ...
    // credit basis minutes (use floor)
    ...
    // compute actual worked minutes (floor)
    ...
    // build blocks_preview (chronological) from workingIntervals to inspect per-block in-punch positions
    $blocks_preview = [];
    ...
    // compute worked coverage per block and detect block_in (first in within/near block)
    ...
    // holiday detection (preserve existing behavior)
    ...
    if ($holiday_stmt) {
        ...
        if ($holiday_result && $holiday_result->num_rows > 0) {
            ...
            if ($apply_multiplier && floor($adjusted_days * 100) / 100 === 1.0) {
                // note: preserving 'round to 2 decimals' semantics but aligned with floor semantics
                $adjusted_days = floor(($adjusted_days * $multiplier) * 100) / 100;
            }
        }
        $holiday_stmt->close();
    }

    // early out detection (preserve)
    $early_out = 0;
    ...
    foreach ($blocks as $b) {
        ...
        if ($block_in > $block_valid_in_end_ts) {
            // use floor to avoid rounding up (aligned with create_attendance)
    ...
    if ($total_deduction_fraction > 0) {
        ...
        $deducted_days_db = round($total_deduction_fraction, 2);       // payroll-visible (2 decimals)
        // final days credited = floor((adjusted_days - deducted_days_db) * 100)/100, min 0.0
    ...
    $resp['original_days_credited'] = $adjusted_days;
