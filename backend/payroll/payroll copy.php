<?php
    ...
    $sql = "
        SELECT 
            ...
            -- ✅ expose override flags directly
    ...
    if ($result->num_rows > 0) {
        ...
        $leaves_data = [];
        $total_paid_leaves = 0;   // ✅ Added
        $total_unpaid_leaves = 0; // ✅ Added
        ...
        $row['total_paid_leaves'] = $total_paid_leaves;     // ✅ Added
        $row['total_unpaid_leaves'] = $total_unpaid_leaves; // ✅ Added
...
// ✅ Loan Deduction Logic
// ✅ Loan Deduction Logic
$loan_deduction_total = 0.00;   // scheduled total across all loans
$loan_deduction_actual_total = 0.00; // sum of all journal "credit" entries in cutoff
$active_loans = [];

$loan_sql = "
    SELECT loan_id, balance, deduction_schedule, payable_per_term, employee_id, loan_amount
    FROM loans 
...
while ($loan = $loan_result->fetch_assoc()) {
    ...
    if ($skip_result->num_rows === 0) {
        // ✅ First attempt: Fetch ALL credit journal entries inside strict cutoff
        $journal_sql = "
        ...
        while ($j = $journal_result->fetch_assoc()) {
            $amt = (float)$j['amount'];
            $actual_deduction += $amt; // ✅ accumulate actual
            $journal_entries[] = [
        ...
        // If no credit entries were found in strict cutoff, try an extended grace window
        // (some systems write journal rows a few days after payroll processing)
        if (count($journal_entries) === 0) {
            // try again with a +7 day grace window
            ...
            while ($j2 = $journal_result2->fetch_assoc()) {
                ...
                $actual_deduction += $amt2; // accumulate
                $journal_entries[] = [
        ...
        // If still no entries found → fallback to payable_per_term (original behavior)
    ...
    $loan_deduction_actual_total += $actual_deduction;

    $active_loans[] = [
        "loan_id"            => $loan_id,
        "deduction_schedule" => $loan['deduction_schedule'],
        "loan_amount"        => number_format((float)$loan['loan_amount'], 2, '.', ''),
        "payable_per_term"   => number_format($payable_per_term, 2, '.', ''), // reference only
...
// ✅ Store both scheduled and actual in row
