...
$is_shift_split = isset($data['is_shift_split']) ? (int)$data['is_shift_split'] : 0; // <-- new
...
try {
    ...
    // Insert into break_time (include is_shift_split)
    $stmt = $conn->prepare("INSERT INTO break_time (work_time_id, break_name, break_start, break_end, valid_break_in_start, valid_break_in_end, valid_break_out_start, valid_break_out_end, is_shift_split) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssssssi", $work_time_id, $break_name, $break_start, $break_end, $valid_in_start, $valid_in_end, $valid_out_start, $valid_out_end, $is_shift_split);
    $stmt->execute();
