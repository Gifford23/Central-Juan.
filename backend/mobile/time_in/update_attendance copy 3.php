...
function normalizeTimeInput($t) {
    ...
    // If user sent "HH:MM" convert to "HH:MM:00"
    if (preg_match('/^\d{2}:\d{2}$/', $t)) $t .= ':00';
    return $t;
}

/* --------- DB helpers (same as create) --------- */
...
if (!isset($data['attendance_id'], $data['attendance_date'], $data['employee_id'], $data['employee_name'])) {
...
// Mobile normalization: if afternoon in around 12:30-13:05 snap to 13:00
...
// Mobile normalization: morning time-out 12:00-12:30 -> 12:00
...
/* --------- SHIFT / BREAKS / WORKING INTERVALS --------- */
...
// compute working intervals = shift minus breaks (chronological)
...
/* --------- early out detection (mobile logic) --------- */
$early_out = 0;
...
/* --------- perform initial UPDATE to attendance --------- */
...
$ust = $conn->prepare($update_sql);
if (!$ust) { echo json_encode(["success"=>false,"message"=>"Prepare failed: ".$conn->error]); exit; }
...
/* --------- LATE DEDUCTION CALC (BLOCK-AWARE) --------- */
$late_debug = [
    'applied' => false,
    'reason' => null,
    'late_minutes' => 0,
...
if ($ok) {
    // build blocks from workingIntervals
    $blocks = [];
    ...
    // accumulate deduction fractions
    $total_deduction_fraction = 0.0;
    $applied_rules = [];
    $last_rule_id = null;

    foreach ($blocks as $b) {
        // first-in punch in block
        $block_in = null;
        foreach ($workedIntervals as $w) {
    ...
    // apply aggregated fraction to DB if any
    if ($total_deduction_fraction > 0) {
        ...
        if ($last_rule_id === null) {
            $update_sql2 = "UPDATE attendance SET days_credited = ?, late_deduction_value = ?, deducted_days = ?, late_deduction_id = NULL WHERE attendance_id = ?";
            $ut = $conn->prepare($update_sql2);
            if ($ut) {
            ...
            $update_sql2 = "UPDATE attendance SET days_credited = ?, late_deduction_value = ?, deducted_days = ?, late_deduction_id = ? WHERE attendance_id = ?";
            $ut = $conn->prepare($update_sql2);
            if ($ut) {
