<?php
// mobile/create_attendance.php (updated)
// Mobile create attendance upgraded to admin/manual-style block-aware logic
// This file merges the mobile endpoint behavior but uses the same robust logic as
// the master create_attendance.php (../create_attendance.php). Keep this file in sync
// with ../create_attendance.php. See ../create_attendance.php for the canonical master.
...
/* --------- main (same behavior as master create_attendance.php) --------- */
$raw = file_get_contents("php://input");
$data = json_decode($raw, true);
error_log('Decoded JSON data (mobile): ' . print_r($data, true));
...
if ($ok) {
    ...
    error_log('Attendance late_debug (mobile): ' . json_encode($late_debug));
    error_log('Response (mobile): ' . json_encode($resp));
    echo json_encode($resp);
} else {
    $errResp = ["success" => false, "message" => ($stmt ? $stmt->error : $conn->error), "late_debug" => $late_debug];
    error_log('Attendance failed (mobile): ' . json_encode($errResp));
