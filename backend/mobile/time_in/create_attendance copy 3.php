...
if (isset($data['attendance_date']) && isset($data['employee_id']) && isset($data['employee_name'])) {
    ...
    // ✅ Normalize morning time-out (12:00 - 12:30 → 12:00)
    ...
    $deduction_morning = ($time_in_morning && strtotime($time_in_morning) > strtotime("09:00:00")) 
        ? getDeduction($conn, $time_in_morning) : 0.0;

    $deduction_afternoon = ($time_in_afternoon && strtotime($time_in_afternoon) > strtotime("13:00:00")) 
        ? getDeduction($conn, $time_in_afternoon) : 0.0;
    ...
    // ✅ Check if it's a holiday and apply multiplier only if adjusted days is full (1.0)
