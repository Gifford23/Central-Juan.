...
import { Edit, AlignJustify, Trash2, Search, Grid3x3, Logs } from 'lucide-react';
...
const Payroll = () => {
    ...
    const filteredPayrollData = useMemo(() => {
        return payrollData.filter((payroll) => {
          const nameMatch = payroll.name?.toLowerCase().includes(searchTerm.toLowerCase());
          const idMatch = payroll.employee_id?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const formatDate = (dateString) => {
    ...
    return formattedDate.replace(/(\w{3})\s(\d{1,2}),\s(\d{4})/, (match, month, day, year) => {
                        ...
                        <div className='flex flex-row gap-x-3 w-fit'>
                            ...
                            >
                                <button 
                                    ...
                                    onClick={() => handlePrint(payrollData.filter((payroll) => selectedPayrolls.includes(payroll.payroll_id)))}
                ...
                <div className='flex gap-x-2 rounded-[15px] mt-4 overflow-y-hidden'>
                    ...
                    {filteredPayrollData.map((payroll) => (
                            <div key={payroll.payroll_id} className="relative flex flex-col p-4 bg-gray-200 payroll_employees_info">
                                ...
                                <div>
                                    <div
                                        onClick={() => toggleDetailsDropdown(payroll)}
                                    >
                                        <div className='flex flex-col cursor-pointer'>
                                            ...
                                            <div className='flex flex-col'>
                                                <p className="font-light text-md">Department: {payroll.department_name}</p>
                                                <p className="font-light text-md">Position: {payroll.position_name}</p>
                                            </div>
                                        </div>

                                        {openDropdownDetails === payroll.payroll_id && (
                                            <div className="mt-4 p-3 bg-white rounded-[10px]">
                                                <p className="text-lg font-semibold">More Information</p>
                                                <div className="grid grid-cols-2 mt-2 place-content-between">
                                                    ...
                                                    <p><strong>Date From:</strong> </p> {new Date(payroll.date_from).toLocaleDateString()}
                                                    <p><strong>Date Until:</strong> </p> {new Date(payroll.date_until).toLocaleDateString()}
                                                    <p><strong>Total Days:</strong> </p> {payroll.total_days}
                                                    <p><strong>Total Salary:</strong> </p> {payroll.total_basic_salary}
                                                </div>
                                            </div>
                                        )}
                                    </div>
                ...
                <div className='mt-4 Glb-table'>
                    ...
                    <div className='Glb-table'>
                        <div className="px-2 Glb-group-1 Glc-tableheader Glc-tableheader-text payroll-grp1">
                            ...
                            <div className="Glb-table-headeroverflow payroll-tableheader">EMPLOYEE DETAILS</div>
                            <div className="Glb-table-headeroverflow payroll-tableheader">POSITION</div>
                            <div className="Glb-table-headeroverflow payroll-tableheader">DAYS</div>
                            <div className="Glb-table-headeroverflow payroll-tableheader">DEDUCTION</div>
                            <div className="Glb-table-headeroverflow payroll-tableheader">TOTAL</div>
                        </div>
                            ...
                            <div className="Glb-table-contentoverflow">
                                <div className="Glb-table-contentoverflow_contentscroll">
                                    ...
                                    <div className='P-Employee-ID w-full text-left text-[13px] flex'>
                                        Employee ID: <strong className='font-bold text-[13px] w-full flex justify-end'>{payroll.employee_id}</strong>
                                    </div>
                                    <div className='P-Payroll-ID w-full text-left text-[13px] flex'>
                                        Payroll ID: <strong className='font-bold text-[13px] w-full flex justify-end'>{payroll.payroll_id}</strong> 
                                    </div>
                                </div>
                            </div>
                            <div className="Glb-table-contentoverflow">
                                <div className="Glb-table-contentoverflow_contentscroll">
                                    <div className="text-[13px] gap-x-10 w-full flex flex-row justify-between items-start">
                                        <div>Post. ID:</div>
                                        <strong className='payroll-content-textelipis font-bold text-[15px] '>{payroll.position_name}</strong> 
                                    </div>
                                    <div className="text-[13px] gap-x-10 w-full flex flex-row justify-between items-start">
                                        <div>Dept. ID:</div>
                                        <strong className='payroll-content-textelipis font-bold text-[15px]'>{payroll.department_name}</strong>
                                    </div>
                                </div>
                            </div>
                            <div className="Glb-table-contentoverflow">
                                <div className="flex flex-col items-center justify-center Glb-table-contentoverflow_contentscroll">
                                    ...
                                    <div className="w-full Glb-table-contentoverflow_contentscroll">
                                        <div className="P-Phil-deduction text-[13px] gap-x-2 w-full flex flex-row justify-between items-end">
                                            ...
                                            <strong className='font-bold text-[14px]'>{payroll.total_days}</strong> 
                                        </div>
                                        <div className="P-SSS-Deduction text-[13px] gap-x-2 w-full flex flex-row justify-between items-end">
                                            ...
                                            <strong className='font-bold text-[14px]'>{payroll.total_overtime_request}</strong>
                            ...
                            <div className="Glb-table-contentoverflow">
                                <div className="Glb-table-contentoverflow_contentscroll">
                                    <div className="P-Phil-deduction text-[13px] gap-x-2 w-full flex flex-row justify-between items-end">
                                        ...
                                        <strong className='font-bold text-[14px]'>{payroll.philhealth_employee_share}</strong> 
                                    </div>
                                    <div className="P-SSS-Deduction text-[13px] gap-x-2 w-full flex flex-row justify-between items-end">
                                        <div className=''>SSS:</div>
                                        <strong className='font-bold text-[14px]'>{payroll.sss_employee_share}</strong>
                                    </div>
                                    <div className="P-PagIbig-Deduction text-[13px] gap-x-2 w-full flex flex-row justify-between items-end">
                                        ...
                                        <strong className='font-bold text-[14px]'>{payroll.pagibig_employee_share}</strong> 
                                    </div>
                                </div>
                            </div>
                            <div className="Glb-table-contentoverflow">
                                <div className="Glb-table-contentoverflow_contentscroll">
                                    <div className="flex justify-between w-full P-Total-Salary gap-x-2">
                                        ...
                                        <strong className='font-bold text-[15px] flex items-end'> {payroll.total_basic_salary}</strong> 
                                    </div>
                                    <div className="P-Total-Deduction text-[13px] gap-x-2 w-full flex flex-row justify-between items-end font-bold">
                                        ...
                                        <strong className='text-[14px]'>
                                            {(
                                                (parseFloat(payroll.philhealth_employee_share) || 0) +
