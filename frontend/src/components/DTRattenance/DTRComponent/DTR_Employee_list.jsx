...
const DTREmployeeLIst = () => {
  ...
  const positionsForSelectedDept = departmentFilter === "all"
    ? [...new Set(employees.map(e => e.position_name).filter(Boolean))]
    : [...new Set(employees.filter(e => e.department_name === departmentFilter).map(e => e.position_name).filter(Boolean))];
...
const filteredData = attendanceData
  ...
  .filter((item) => {
    ...
    const matchesType = typeFilter === "all" || item.employee_type?.toLowerCase() === typeFilter.toLowerCase();
    const matchesDept = departmentFilter === "all" || item.department_name === departmentFilter;
    const matchesPos = positionFilter === "all" || item.position_name === positionFilter;
    const matchesBranch = branchFilter === "all" || item.branch_name === branchFilter;

    return matchesSearch && matchesStatus && matchesType && matchesDept && matchesPos && matchesBranch;
  });
...
const breadcrumbItems = [
  ...
  const renderStatusBadge = (status) => {
    ...
    switch (s) {
      case "active":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 ring-1 ring-emerald-200">Active</span>;
      case "inactive":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-800 ring-1 ring-slate-200">Inactive</span>;
      case "on leave":
      case "on_leave":
      case "leave":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 ring-1 ring-amber-200">On Leave</span>;
      case "suspended":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 ring-1 ring-red-200">Suspended</span>;
      default:
        // fallback: a distinct cool blue
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-sky-100 text-sky-800 ring-1 ring-sky-200">{status || "Unknown"}</span>;
    }
  };
  ...
  const renderTypeBadge = (type) => {
    ...
    switch (t) {
      case "regular":
        return (
          <span className={`${baseClass} bg-orange-50 text-orange-900 ring-orange-200`}>
            Regular
          </span>
        );
      ...
      case "parttime":
        return (
          <span className={`${baseClass} bg-purple-50 text-purple-900 ring-purple-200`}>
            Part-time
          </span>
        );
      ...
      case "ojt":
        return (
          <span className={`${baseClass} bg-teal-50 text-teal-900 ring-teal-200`}>
            OJT
          </span>
        );
      ...
      case "contractual":
        return (
          <span className={`${baseClass} bg-rose-50 text-rose-900 ring-rose-200`}>
            Contractual
          </span>
        );
      ...
      case "project based":
        return (
          <span className={`${baseClass} bg-indigo-50 text-indigo-900 ring-indigo-200`}>
            Project-Based
          </span>
        );
      ...
      case "":
        return (
          <span className={`${baseClass} bg-stone-50 text-stone-800 ring-stone-200`}>
            N/A
          </span>
        );
      ...
      default:
        return (
          <span className={`${baseClass} bg-amber-50 text-amber-900 ring-amber-200`}>
            {type || "Unknown"}
          </span>
        );
    }
        ...
        <Box className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              startIcon={updateAllLoading ? <CircularProgress size={16} /> : null}
            >
        ...
        >
          ...
          {positionsForSelectedDept.map((pos) => (
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              <Select labelId="status-filter" value={statusFilter} label="Status" onChange={(e) => setStatusFilter(e.target.value)}>
            ...
            <FormControl size="small" sx={{ minWidth: 160 }}>
              ...
              <Select labelId="type-filter" value={typeFilter} label="Type" onChange={(e) => setTypeFilter(e.target.value)}>
...
<div className="flex-1 flex flex-col">
  ...
  <div className="flex flex-col gap-1 mt-1 text-sm text-gray-600">
    ...
    <span>Pos: <strong>{item.position_name}</strong></span>
    {item.branch_name && <span>Branch: <strong>{item.branch_name}</strong></span>}
    <span>ID: <strong>{item.employee_id}</strong></span>
  </div>
    ...
    {renderStatusBadge(item.status)}
                        ...
                        <div className="flex items-center gap-6">
                          <div className="text-sm text-left">
                            <div>1st Half: <strong className="font-mono">{(firstHalfDays || 0).toFixed(2)}</strong></div>
                            <div>2nd Half: <strong className="font-mono">{(secondHalfDays || 0).toFixed(2)}</strong></div>
                          </div>
