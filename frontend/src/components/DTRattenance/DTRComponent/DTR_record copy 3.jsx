...
const handleClear = async (record, index) => {
  ...
  ["time_in_morning", "time_out_morning", "time_in_afternoon", "time_out_afternoon"].forEach(field => {
    if (!cleanedRecord[field]) {
      ...
      // normalize to HH:MM or HH:MM:SS consistently for debugging
      // if user input is "HH:MM", add ":00" so server sees consistent format
      if (/^\d{2}:\d{2}$/.test(cleanedRecord[field])) {
        cleanedRecord[field] = cleanedRecord[field] + ":00";
      }
    }
  });
  ...
  try {
    ...
    console.log("🛰️ [DEBUG] Sending attendance save request", {
      url,
      method,
      cleanedRecord,
    });
    ...
    const rawText = await response.text(); // raw response (useful if PHP emits warnings)
    console.log("📥 [DEBUG] Raw response text from server:", rawText);

    let result;
    try {
      result = JSON.parse(rawText);
    } catch (parseError) {
      console.error("❌ JSON parse error:", parseError.message, " — rawText:", rawText);
      throw new Error("Server did not return valid JSON. Check PHP errors on server.");
    }

    console.log("✅ [DEBUG] Parsed result object:", result);

    if (result.success) {
      Swal.fire("Success!", `Attendance record for ${record.attendance_date} saved successfully.`, "success");

      // POST-UPDATE VERIFY: re-fetch the month's records and show what's returned for this exact date
      try {
        const verifyResp = await fetch(`${BASE_URL}/attendance/get_attendance.php?employee_id=${record.employee_id}&month=${selectedMonth + 1}&year=${selectedYear}`);
        const verifyJson = await verifyResp.json();
        console.log("🔁 [DEBUG] Refetched month data:", verifyJson);

        if (verifyJson && Array.isArray(verifyJson.data)) {
          const serverRecord = verifyJson.data.find(r => r.attendance_date === record.attendance_date);
          console.log("🔍 [DEBUG] Server record for saved date:", serverRecord);
          // Extra micro-checks:
          if (serverRecord) {
            console.log("⏱️ time_in_afternoon (server):", serverRecord.time_in_afternoon);
            console.log("⏱️ time_out_afternoon (server):", serverRecord.time_out_afternoon);
            console.log("typeofs:", {
              tiAf: typeof serverRecord.time_in_afternoon,
              toAf: typeof serverRecord.time_out_afternoon
            });
          } else {
            console.warn("⚠️ [DEBUG] Server did not return a record matching attendance_date after save.");
          }
        }
      } catch (verifyErr) {
        console.error("❌ [DEBUG] Error verifying saved record:", verifyErr);
      }
