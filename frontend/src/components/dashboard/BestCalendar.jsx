  ...
  return (
    ...
    >
      <div className="flex items-center justify-between w-full">
        <div
          className={`text-xs font-semibold ${isHoliday ? "text-red-700" : inMonth ? "text-slate-700" : "text-slate-400"}`}
        >
          {format(date, "d")}
        </div>
        {today && (
          <div className="text-[10px] px-2 py-0.5 bg-sky-50 text-sky-700 rounded-full">
            Today
          </div>
        )}
      </div>
      ...
      <div className="mt-1 w-full flex flex-col gap-1">
        ...
        {eventsForDay?.slice(0, 3).map((ev) => (
          ...
          >
            ...
            />
            <div className="text-[11px] truncate font-medium" aria-hidden>
              {ev.time ? `${ev.time} • ` : ""}
              {ev.title}
            </div>
          </div>
        ))}
        {eventsForDay && eventsForDay.length > 3 && (
...
const MonthHeader = ({ currentDate, onPrev, onNext, onToday, onJumpTo }) => (
  <div className="flex items-center justify-between gap-3">
    ...
    <div className="flex items-center gap-2">
      <button
        onClick={onToday}
        className="px-3 py-1 text-sm bg-sky-600 text-white rounded shadow hover:opacity-95"
      >
        Today
      </button>
      <select
...
const BestCalendar = ({
  ...
  const handleJumpTo = (iso) => {
    if (!iso) return;
    setCurrentMonth(startOfMonth(new Date(iso)));
  };
        ...
        <div className="grid grid-cols-7 gap-3 px-5 pb-5">
          ...
          {weeks.map((day) => {
            ...
            const birthdays = employees.filter((emp) => {
              ...
              return (
                dob.getDate() === day.getDate() &&
                dob.getMonth() === day.getMonth()
              );
            });
            ...
            return (
              <div key={key} className="relative">
                <div className="p-1">
                  ...
                  />
                  {holiday && (
                    <div className="absolute top-1 left-1 text-[10px] text-red-700 font-semibold"></div>
                  )}
                </div>
                    ...
                    >
                      ...
                      {birthdays.length > 0 && (
                        <div className="mb-2 text-sky-600 font-medium flex flex-col gap-1">
                          {birthdays.map((b) => (
                            <div key={b.employee_id}>
                              🎂 {b.first_name} {b.last_name}
                            </div>
                          ))}{" "}
                          {/* full name shown in popup */}
                        </div>
                      )}
                      ...
                      {eventsForDay.length > 0 ? (
                        <div className="flex flex-col gap-2">
                          {eventsForDay.map((ev) => (
                            <div
                              key={ev.id}
                              className="p-2 rounded border border-slate-100 flex items-start gap-2"
                            >
                              <div
                                className="w-2 h-2 rounded-full mt-1"
                                style={{
                                  backgroundColor: ev.color || "#06b6d4",
                                }}
                              />
