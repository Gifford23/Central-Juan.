...
const TaskManagement = ({
  ...
  return (
    <div
      className="flex flex-col p-4 bg-white shadow-lg rounded-2xl w-full max-w-full md:max-w-[400px] lg:w-full mx-auto"
      style={{ height: "40vh" }}
    >
      {/* Header controls */}
      ...
      {!showAddTask && !showTasks && (
        <div className="grid flex-grow min-h-0 grid-cols-7 gap-1 mb-2 overflow-y-auto text-center text-gray-600">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <div
              key={day}
              className={`text-[10px] font-medium ${day === "Sun" ? "text-red-500" : ""}`}
            >
              {day}
            </div>
          ))}
            ...
            return (
              <div
                ...
                title={
                  holidayForDay
                    ? holidayForDay.name
                    : birthdaysForDay.length > 0
                      ? birthdaysForDay.map((b) => b.name).join(", ")
                      : ""
                }
              >
                {format(day, "d")}
