...
function SessionGate() {
  ...
  const { roles, loading: rolesLoading } = useRoles(); // This uses your useAuthRoles hook
  ...
  // 3. DEFENSIVE CODING: Check if roles is an array before mapping
  const safeRoles = Array.isArray(roles) ? roles : [];

  // Normalize role names (safe uppercase)
  const normalizedRoles = safeRoles.map(
    (r) => r?.role_name?.toUpperCase() ?? "",
  );
    ...
    <Router>
      <Routes>
        ...
        >
          ...
          <Route
            path="EmployeeGridViewPage"
            element={<EmployeeGridViewPage />}
          />
          ...
          <Route
            path="PagibigContributionTable"
            element={<PagibigContributionTable />}
          />
          <Route path="pagibigContribution" element={<PagibigContribution />} />
          <Route
            path="PhilhealthContributionTable"
            element={<PhilhealthContributionTable />}
          />
          <Route
            path="PhilhealthContribution"
            element={<PhilhealthContribution />}
          />
          ...
          <Route
            ...
            element={<NotificationRequestPage />}
          />
          ...
          <Route
            path="commission-per-employee"
            element={<CommissionPerEmployee />}
          />
        </Route>
